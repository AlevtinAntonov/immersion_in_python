...
# Строку в верхнем регистре в остальных случаях *обработку исключений использовать нельзя

# user_input = input('Введите что-нибудь из условия: ')
user_input = ".5665654654654"

if user_input.isdecimal():
    result = 'Целое'
    val = int(user_input)
# elif user_input.count('.') == 1 and user_input[0] != '.':
elif user_input.replace('.', '').replace('-', '').isdecimal() and user_input.count('.') == 1 and user_input.count('-') <= 1 and user_input[-1] != '.' and user_input[1:].count('-') == 0:
    if user_input[0] == '-':
