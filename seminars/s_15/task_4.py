...
logging.basicConfig(filename='./log/task4.log',
                    filemode='w',
                    encoding='utf-8',
...
def text_to_date(text: str):
    ...
    for day in range(1, 31 + 1):
        ...
        if rezult.weekday() == weekday:
            count_week += 1
            if count_week == count:
                UserDate = namedtuple('UserDate', ['day', 'month', 'year'])
                inp_user_date = UserDate(count, day, year)
...
if __name__ == '__main__':
    ...
    print('2-й четверг ноября:', text_to_date('2-й четверг ноябрь'))
    print('1-я среда мая:', text_to_date('1-я среда май'))
